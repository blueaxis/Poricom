...
class RibbonTab(QWidget):
    ...
    def loadButtonConfig(self, buttonName, buttonConfig):
        ...
        w = self.parent.frameGeometry().height()*config["TBAR_ISIZE_REL"]*buttonConfig["icon_w"]
        h = self.parent.frameGeometry().height()*config["TBAR_ISIZE_REL"]*buttonConfig["icon_h"]
        ...
        if (exists(path)):
            icon = QIcon(path)
        else: icon = QIcon(config["TBAR_ICON_DEFAULT"])
        ...
        tooltip = f"<h3 style='margin-bottom: 4px;'>{buttonConfig['help_title']}\
            </h3><p style='margin-top: 0;'>{buttonConfig['help_msg']}</p>"
...
class Ribbon(QTabWidget):
    def __init__(self, parent = None, tracker = None):
        ...
        h = self.parent.frameGeometry().height() * config["TBAR_ISIZE_REL"] * config["RBN_HEIGHT"]
