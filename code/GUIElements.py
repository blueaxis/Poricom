...
class RibbonTab(QWidget):
    ...
    def loadButtonConfig(self, b_name, b_config):
        ...
        w = self.parent.frameGeometry().height()*config["TBAR_ISIZE_REL"]*b_config["icon_w"]
        h = self.parent.frameGeometry().height()*config["TBAR_ISIZE_REL"]*b_config["icon_h"]
        m = config["TBAR_ISIZE_MARGIN"]

        icon = QIcon()
        path = config["TBAR_ICONS"] + b_config["path"]
        if (exists(path)):
            icon = QIcon(path)
...
class PageNavigator(RibbonTab):
    ...
    def __init__(self, parent=None, tracker=None):
        ...
        for func_name, func_cfg in config["MODE_FUNCS"].items():
            self.loadButtonConfig(func_name, func_cfg)
...
class Ribbon(QTabWidget):
    def __init__(self, parent=None, tracker=None):
        ...
        #TODO: add keyboard shortcut using name scheme
        for tab_name, tools in config["TBAR_FUNCS"].items():
